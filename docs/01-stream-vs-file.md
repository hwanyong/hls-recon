...
> 제1부 문제의 성립 · 1/3
> 선행: 없음 — 교재의 출발점 · 후행: 제2장(HTTP 위에서 스트리밍을 흉내내기)
...
삼으면 밀린 자막이 스스로 기준을 끌고 가 검사가 항상 통과한다(`report.py:339-342`).
...
![스트림에서 파일로 — 변환이 버리는 것](images/01-stream-to-file.svg)
...
이것이 `hls-recon` 이 재조립 자체는 ffmpeg 에 위임하면서도(`assemble.py:1-6`) 수신은
...
![총 길이는 같고 중간이 비어 있다](images/01-timeline-hole.svg)
...
이상이면 FAIL 이다(`report.py:262-278`).
...
> 사이 패킷이 유실됐다는 뜻이다(`tsanalyze.py:104-119`). 4비트뿐이므로 정확히 16의
...
- **§1.6.1 의 공격 경로는 추론이지 실측이 아니다.** 이 장에서 실제로 측정한 것은
  ...
  선택적 차단이 실제 CDN 환경에서 얼마나 쉬운지, 재시도 정책(`fetch.py:196-206`)이
  그것을 얼마나 흡수하는지는 재현하지 않았다.
- **`EXT-X-DISCONTINUITY` 예외의 완결성은 검증하지 않았다.** `report.py:310` 은 선언된

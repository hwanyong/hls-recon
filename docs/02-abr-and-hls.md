# 제2장 — HTTP 위에서 스트리밍을 흉내내기

**ABR 과 HLS 의 발명**

> 제1부 문제의 성립 · 2/3
> 선행: 제1장(스트림과 파일 — 두 존재론) · 후행: 제3장(RFC 8216 의 2계층 간접 참조)
...
![상태가 어디에 있는가](images/02-where-state-lives.svg)
...
![ABR 의 반응 지연](images/02-abr-lag.svg)
...
그것을 그대로 보관한다(`playlist.py:156`, `playlist.py:297-298`). 판정에도 쓰인다 —
실측 길이와 선언 길이의 차이가 `TARGETDURATION` 이상이면 세그먼트 하나 이상이 통째로
어긋난 것이므로 **FAIL** 로 판정한다(`report.py:264-272`).
...
`height` 만은 계산된 값이다(`playlist.py:115-122`). `RESOLUTION` 문자열에서
...
|---|---|---|---|
| `seq` | 송출 전역의 위치 | `#EXT-X-MEDIA-SEQUENCE` + 누적 (`playlist.py:300-302`, `248`) | **AES-128 복호화가 깨진다.** IV 속성이 없으면 이 번호가 128비트 IV 가 된다 |
| `index` | 이 목록 안의 0-기반 순번 | `len(pl.segments)` (`playlist.py:241`) | 보고·진단의 지칭이 어긋난다 |
...
카운터(`tsanalyze.py:71-121`), 컨테이너 판별(`tsanalyze.py:20-37`), 타임라인 갭
스캔(`probe.py:191-233`)은 전부 **매니페스트가 답해 주지 않는 질문을 바이트에서
직접 되찾는 작업**이다.
...
구조를 이용한 것이다. `parse()` 한 번으로 전체 목록을 얻고(`playlist.py:207-337`),
`fetch.get_many` 로 병렬 수신한다(`fetch.py:223-243`). 전용 프로토콜에서는 세션을
...
- **마스터/미디어 혼합 문서를 거부하지 않는다.** `parse()` 는 같은 문서에
  ...
  거부는 상위에서 `media.is_master` 하나만 본다(`cli.py:192-193`).
...
- **라이브의 시간적 성질은 스냅샷 한 장으로만 관찰했다.** 매니페스트를
  ...
  (`cli.py:399-401`, README 의 알려진 한계).
...
5. `Variant`(`playlist.py:102-132`)와 `Segment`(`playlist.py:135-146`)는 **직교하는 두
...
`playlist.py:185-204` 와 `cli.py:172-196` 에서 읽는다.

# 제3장 — RFC 8216 의 2계층 간접 참조

**마스터/미디어 분리가 만드는 것과 잃는 것**

> 제1부 문제의 성립 · 3/3
> 선행: 제2장(HTTP 위에서 스트리밍을 흉내내기) · 후행: 제4장(무상태성과 무결성 보증의 부재)
...
`playlist.py:207-337` 의 `parse` 는 마스터와 미디어를 **같은 진입점에서** 받는다.
...
(`playlist.py:149-161`). 종류는 필드로 주어지지 않고 파싱 도중 부수효과로 켜진다.
...
`EXT-X-BYTERANGE` 의 오프셋 생략 규칙(`playlist.py:327-329`)이고, 제6장에서 다룬다.
...
둘뿐이고(`cli.py:208` · `cli.py:399`), 둘 다 `_resolve_media` 가 이미 풀어낸 **미디어**
플레이리스트를 받는다(`cli.py:535` · `cli.py:583`). `is_master` 가 `True` 인 객체가
...
`ValueError("마스터 플레이리스트에 variant 가 없다")` 로 떨어진다(`playlist.py:190`).
...
![2단 간접과 base URL 승계](images/03-two-tier.svg)

*그림 3-1 — 상대 URI 는 그것이 적힌 문서의 주소로 절대화된다. ①이 낳은 주소가 ②를 파싱할 때의 base 가 된다.*

| 층 | 호출 지점 | `base_url` |
|---|---|---|
| ① 마스터 | `cli.py:138` | 사용자가 준 소스 주소 |
| ① 마스터(로컬 파일) | `cli.py:145` | `p.as_uri()` — `file://` 스킴 |
| ② variant | `cli.py:189` | `chosen.uri` — **①이 낳은 주소** |
| ③ 자막 트랙 | `cli.py:274` | `track.uri` — **①이 낳은 또 다른 주소** |
...
같은 이유로 `cli.py:189` 의 `base_url=chosen.uri` 도 필수다. variant 가 마스터와 다른
...
(`playlist.py:115-122`)도 같은 사실의 귀결이다 — **항상 존재하는 정렬 키는 대역폭뿐이다.**
...
> 남긴다(`cli.py:135` · `140` · `147` · `187` · `191`). 그런데 `pick_variant` 를 부르는
> 자리(`cli.py:183`)에는 `try` 가 없고, `_run_one` 에도 `main` 에도 없다. 결과적으로
> `--height 720` 을 잘못 준 사용자는 공들여 만든 "사용 가능: [1080, 480]" 메시지를
> **파이썬 스택 트레이스에 실려서** 본다. 시리즈 경로는 더 나쁘다 — `cli.py:757` 의
...
(`cli.py:192-193`). 자기 자신을 다시 부르는 두 줄만 쓰면 중첩을 "지원"할 수 있는데,
그렇게 하지 않은 근거가 셋이다.
...
`cli.py:186-187` 은 선택한 variant 를 받지 못하면 즉시 `SystemExit` 다. 다음 후보로
...
![그룹 참조 — 이름으로 잇는 세 번째 간접](images/03-group-ref.svg)
...
| 참조 | 형식 | 대상 | 해석 시점 | 어긋나면 |
|---|---|---|---|---|
| `EXT-X-STREAM-INF` → 미디어 | URI(다음 줄) | 다른 문서 | HTTP 요청 시 | `SystemExit` — 즉시 중단 (`cli.py:186-187`) |
| `EXT-X-MEDIA` → 자막 플레이리스트 | URI(속성) | 다른 문서 | 자막 단계 | 그 트랙만 실패, 영상은 진행 (`cli.py:275-277`) |
...
(`subtitles.py:82` 의 `external = [t for t in tracks if not t.is_embedded]`).
...
플레이리스트 URL 을 직접** 준 경우(`cli.py:176-177`)에는 마스터가 존재하지 않는다.
그러면 `subtitle_tracks()` 는 `[]` 를 돌려준다(`cli.py:162-163`).
...
같은 `Source` 가 자막 메우기 경로(`cli.py:756-766`)에서도 그대로 쓰인다. 2계층 해석을
...
모른다. 그래서 이 코드는 진단을 클라이언트에 둔다 — `_diagnose`(`cli.py:57-102`)가
①층(`cli.py:140`)과 ②층(`cli.py:191`) **양쪽에서** 불린다. 층마다 실패 모드가 달라도
...
`_resolve_media` 의 `fetcher.get(chosen.uri)`(`cli.py:185`)는 마스터를 받은 요청과
...
이 재시도는 ①층에만 있다. ②층 요청(`cli.py:185`)에는 없다 — 그때는 이미 `fetcher.headers`
에 Referer 가 들어 있으므로 필요가 없다는 판단이다. **헤더 상태가 층을 가로질러
승계되기 때문에 성립하는 최적화**이고, `fetcher.headers` 를 단일 출처로 유지하겠다는
결정(`cli.py:526-529`)이 그 전제다. 사본을 넘기면 층마다 다른 헤더로 요청하게 된다.
...
`cli.py:192-193` 의 중첩 마스터 거부는 기능 결정이자 **보안 결정**이다. 참조 그래프를
...
- **`pick_variant` 의 `ValueError` 가 `SystemExit` 으로 변환되지 않는다.** `cli.py:183` 에
  `try` 가 없어 후보 불일치가 스택 트레이스로 새어 나오고, 시리즈 경로의
  `except SystemExit`(`cli.py:757`)은 이것을 잡지 못해 **한 회차의 실패가 전체 실행을
  중단시킨다.** 다른 모든 실패 경로가 지키는 규율을 이 한 곳만 지키지 않는다.
  이 장을 쓰면서 발견했고, 고치지는 않았다.
- **`max_bandwidth` 의 비대칭.** `pick_variant`(`playlist.py:200-203`)는 상한 아래 후보가
...
- **그룹 dangling 이 무음이다.** §3.6.2 의 세 번째 경우에서 자막이 사라지는데 아무
  메시지도 나오지 않는다. `cli.py:557` 의 경고는 `all_subs` 가 비면 도달하지 않는다.
  고치려면 `Source.subtitle_tracks()` 가 "그룹은 지정됐는데 매칭이 0"인 경우를 구별해
  돌려주어야 한다.
- **base 승계를 검증하는 테스트가 없다.** `tests/run.sh:59-66` 의 `multi/` 픽스처는
  마스터와 variant 가 **같은 디렉터리**에 있어, `cli.py:189` 의 `base_url=chosen.uri` 를
...
- **마스터–variant 사이의 TOCTOU 를 검사하지 않는다.** 두 요청 사이에 서버가 후보 집합을
  ...
  좁히지도 않는다.
- **`TYPE=AUDIO` 는 표시만 한다.** 그룹 참조를 읽어 목록에 출력하지만(`cli.py:219`) 별도
...
4. **`EXT-X-MEDIA` 의 그룹 참조는 URI 가 아니라 이름으로 잇는 세 번째 간접**이고, 유일하게
   ...
   `tracks()` 가 **빈 목록을 조용히** 돌려준다(`playlist.py:177-183`).
5. **중첩 마스터 거부는 세 겹의 근거를 갖는다** — 규격 위반, 종료 보장, 의미의 부재
   (`cli.py:192-193`). 세 번째가 가장 무겁다. 지원 가능한 것과 지원해야 하는 것은 다르다.
6. **`Source` 는 내려가면서 상위 문맥을 버리지 않기 위해 존재한다**(`cli.py:150-169`).
...
보증하지 않는지를 `fetch.py:107-215` 에서 읽고, "받아졌다"가 왜 "옳게 받아졌다"가 아닌지를
전송 계층에서 확정한다.

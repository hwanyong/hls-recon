# 제4장 — 무상태성과 무결성 보증의 부재

**HTTP 는 무엇을 보증하고 무엇을 보증하지 않는가**

> 제2부 전송: HTTP 의 태생적 한계 · 4/8
> 선행: 제3장(RFC 8216 의 2계층 간접 참조) · 후행: 제5장(상태 코드의 의미론적 붕괴)
...
![계층별 무결성 검사의 범위](images/04-guarantee-ladder.svg)
...
![요청별 결과를 값으로 남기고 가로질러 세는 구조](images/04-instrumented-gap.svg)
...
| 압축 해제 실패 | **안 함** (`break`, `fetch.py:177-180`) | 같은 서버가 같은 손상된 본문을 다시 준다 |
...
한 가지 덧붙일 것이 있다. `IncompleteRead` 는 `fetch.py:202` 의 광범위한
...
`fetch.py:177-180` 에서 `ok=False` 로 이어진다. 즉 이 코드에서 **`Content-Encoding:
gzip` 인 응답은 `identity` 인 응답보다 무결성 보증이 강하다.** 설계된 것이 아니라
압축 포맷의 부수 효과이며, 그래서 Range 요청 경로(`fetch.py:155-159` 가
...
`_run_segments` 는 실패한 세그먼트를 건너뛸 뿐 중단하지 않는다(`cli.py:452-455`).
...
무작위 바이트열이 PKCS#7 패딩 검사(`decrypt.py:56`)를 통과할 확률은 약 1/256 이다 —
...
| 층 | 검사 | 잡는 범위 |
|---|---|---|
| 전송 | `len(failed)/len(fetches)` (`report.py:167-173`) | 요청이 실패한 결손 |
| 산출물 | PTS 갭 스캔 (`report.py:303-324`, 제21장) | 요청은 성공했으나 타임라인에 구멍이 난 경우 |
...
- **요청 목록 자체의 완결성은 검사하지 않는다.** 기준선 N 이 파싱된 플레이리스트에서
  나오고, 선언 길이도 같은 곳에서 나온다(`cli.py:414-415`).
  ...
  잘려 나가면 `is_live` 가 참이 되어(`playlist.py:164-166`) LIVE 로 보고되고 처리
  경로가 달라진다. 무결성을 위해 설계된 장치가 아니므로 여기에 기대면 안 된다.
  (측정 대상이 측정 기준을 끌고 가는 이 문제의 일반형은 제38장에서 다룬다.)
- **`attempts` 계측이 실패 건에서 부정확하다.** `fetch.py:212` 가 `attempts=self.retries`
  를 고정으로 넣으므로, 404 로 첫 시도에서 `break` 한 요청도 "3회 시도"로 기록된다.
  현재 리포트가 이 값을 성공 건에만 쓰기 때문에(`report.py:162` 의 `f.ok and
...
- **`Fetcher.get` 은 예외를 던질 수 있다.** `normalize_url` 이 재시도 루프 **밖**에
  있어(`fetch.py:151`) 잘못된 형태의 URL 은 `FetchResult` 가 되지 못한다. 실측:
...
- **압축 경로가 비압축 경로보다 우연히 강하다.** gzip 의 CRC-32 트레일러 덕분인데,
  ...
  강제하므로(`fetch.py:155-159`) 그 보호를 잃는다. `EXT-X-BYTERANGE` 세그먼트가 많은
...
6. **빈칸은 프로토콜이 아니라 자료구조로 메워진다.** 요청 하나를 `FetchResult` 한
   건으로 만들고(`fetch.py:73-104`), 실패도 값으로 리스트에 남기고, 그 리스트를
   가로질러 센다(`report.py:160-173`). **실패를 예외로 던졌다면 셀 대상 자체가
   사라진다.**

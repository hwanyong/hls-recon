...
> 제2부 전송: HTTP 의 태생적 한계 · 6/8
> 선행: 제5장(상태 코드의 의미론적 붕괴) · 후행: 제7장(URL 정규화와 멱등성)
...
그래서 이 저장소는 진단을 전송 계층까지 끌고 내려간다(`cli.py:57-102`). 그 함수를
실제로 호출해 얻은 출력이다.
...
브라우저 UA 를 쓰면(`fetch.py:20-23`) 서버가 압축을 돌려줄 확률이 올라간다. 즉 **한 축의
...
![같은 범위 요청이 가리키는 두 좌표계](images/06-range-vs-encoding.svg)
...
즉 "압축된 표현의 부분"은 규격상 말이 되지만 **쓸모가 없다.** 그래서 `fetch.py:159` 는
...
해제기는 모르는 코딩을 만나면 조용히 넘기지 않고 예외를 던진다(`fetch.py:70`).
...
`break` 다 — 재시도 루프를 **빠져나간다.** 이것은 `fetch.py:199-201` 의 "4xx 는
...
본문이 중간에 끊겼다면 `resp.read()` 자체가 예외를 내고 일반 예외 경로(`fetch.py:202`)로
...
`fetch.py:159` 의 `identity` 강제는 `headers["…"] = …` 가 아니라 `setdefault` 다. 사용자가
직접 지정한 헤더가 이긴다는 뜻이다(`fetch.py:146-150`의 원칙). **이 방어는 기본값이지
불변식이 아니다.** 직접 확인한 결과다.
...
해시가 해제 후 본문에 대해 계산된다는 점(`fetch.py:193`)은 별도로 짚을 만하다.
...
않는다.** 리포트의 세그먼트 중복 검사(`report.py:213`)는 해시 집합의 크기를 비교하는데,
...
![압축 해제 증폭 경로](images/06-decompression-amplification.svg)

*그림 6-2 — 회선에서 메모리까지의 경로와, 상한을 걸 수 있는 유일한 지점.*

**중요한 것은 이것이 압축이 만든 취약점이 아니라는 점이다.** `raw = resp.read()`
(`fetch.py:172`) 에도 상한이 없다. 압축 없이도 서버는 무한히 큰 본문을 흘려보내 같은
...
던지는 예외가 `ValueError` 인 것도 의도적이다. `fetch.py:177` 의 `except Exception` 이
...
| 세그먼트 고유성(SHA-256) | **걸릴 수 있다** — WARN | 모든 세그먼트가 같은 전체 파일이면 해시가 전부 같다(`report.py:213-218`) |
...
- **`EXT-X-BYTERANGE` 경로 자체가 회귀 테스트에 없다.** `tests/run.sh` 는 이 태그를 쓰는
  스트림을 만들지 않는다(전수 확인). 파싱(`playlist.py:327-329`)과 요청 조립
  (`fetch.py:155-161`)은 코드로만 존재하고 실행으로 고정돼 있지 않다.

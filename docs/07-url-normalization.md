...
> 제2부 전송: HTTP 의 태생적 한계 · 7/8
> 선행: 제6장(콘텐츠 협상의 부작용) · 후행: 제8장(병렬성·재시도·계측)
...
같다 — "여기가 URI 를 낳는 유일한 지점이다"(`playlist.py:31`).
...
![상대 URI 와 기준 URL 이 한 함수로만 들어가고 거기서 나온 값을 네 소비자가 나눠 쓴다](images/07-uri-birth.svg)
...
| 호출 지점 | 무엇의 URI 인가 |
|---|---|
| `playlist.py:229` | 세그먼트와 variant (태그가 아닌 줄 = 직전 태그가 가리키는 URI) |
| `playlist.py:282` | `#EXT-X-MEDIA` 의 자막·오디오 트랙 |
| `playlist.py:313` | `#EXT-X-KEY` 의 복호화 키 |
| `playlist.py:325` | `#EXT-X-MAP` 의 초기화 세그먼트 |
...
| 소비자 | 누적 적용 횟수 | 경로 |
|---|---|---|
| HTTP 세그먼트 수신 | **2회** | `_absolute` → `fetch.py:151` |
| 복호화 키 요청 | **2회** | `_absolute` → `fetch.py:151` |
| ffmpeg 입력 — 마스터에서 고른 variant | **1회** | `_absolute` 만 (`cli.py:195` 의 `chosen.uri`) |
| ffmpeg 입력 — 사용자가 미디어 플레이리스트를 직접 준 경우 | **0회** | `cli.py:177` 이 입력 문자열을 그대로 `media_url` 로 삼는다 |
...
준 문자열을 그대로 base URL 로 돌려주고(`cli.py:138`), 그 값이 마스터가 아니면
`media_url` 이 되어 ffmpeg 로 간다(`cli.py:177`). 세그먼트 주소는 `_absolute` 가
...
> `fetch.py:151` 을 지우면, 명령줄에서 날것 한글 주소를 넣은 실행만 §7.1.1 의
...
![이름 하나가 요청줄이 되기까지의 세 단계](images/07-encode-once.svg)
...
| 3 | (경계에서) `normalize_url` | 다시 URI 로 되돌린다 — `fetch.py:151` 이 한다 |
...
**모든 `key.uri` 가 `playlist.py:313` 을 통과한 값**이라는 사실뿐이다. 같은 키를
...
![요청·검증기·파일 계층에서 누적 디코드 횟수가 다르다](images/07-decode-mismatch.svg)
...
- **"ffmpeg 도 입력을 열지 못한다"는 주석이 재현되지 않았다.** `fetch.py:41-42` 의
...
- **`series.discover` 의 중복 제거 키는 정규화 전 값이다.** `page_url` 을
  `urljoin` 결과 그대로 `seen` 집합에 넣는다(`series.py:171-174`). 같은 회차가 두
...
5. **URI 를 낳는 지점은 하나다**(`playlist.py:30-38`). 플레이리스트 상대 경로·

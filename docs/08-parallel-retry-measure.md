...
> 제2부 전송: HTTP 의 태생적 한계 · 8/8
> 선행: 제7장(URL 정규화와 멱등성) · 후행: 제9장(핫링크 차단의 해부)
...
| 결정 | 코드 위치 | 한 줄 요약 |
|---|---|---|
| **재시도 분류** | `fetch.py:139-215` `_send` | 실패를 "다시 하면 달라질 수 있는 것"과 아닌 것으로 나눈다 |
| **순서 복원** | `fetch.py:223-243` `get_many` | 완료 순서로 받되 제출 시점의 자리로 되돌린다 |
| **분위수 계측** | `report.py:56-61` `_quantile` | 평균이 아니라 p50·p95 로 지연을 본다 |
...
> 에서 0.8초 → 1.6초가 된다(`fetch.py:205-206`).
...
부수 효과로, 반환되는 `FetchResult.url`(`fetch.py:182`)은 정규화 **후**의 문자열이다.
리포트에 남는 주소가 실제로 회선에 나간 주소와 같다는 뜻이고, 이것이 없으면 사후
재현이 어긋난다.

### 8.3.2 루프는 세 갈래로만 갈린다

![한 번의 시도가 끝났을 때 재시도 루프가 갈리는 세 갈래](images/08-retry-classification.svg)
...
루프를 빠져나간 뒤 `fetch.py:208-215` 의 공통 실패 반환으로 합류해야 하기 때문이다.
성공은 루프 안에서 즉시 `return` 하고(`fetch.py:181-195`), 실패는 종류를 불문하고
...
| 분기 | 발생 조건 | 루프 동작 | 앵커 |
|---|---|---|---|
| 성공 | 응답 수신 + 압축 해제 성공 | `return` (즉시) | `fetch.py:181-195` |
| 본문 해제 실패 | `Content-Encoding` 해제 중 예외 | `break` — 재시도 안 함 | `fetch.py:177-180` |
| 영속 실패 | 4xx 중 `408`·`429` 제외 | `break` — 재시도 안 함 | `fetch.py:199-201` |
| 일시 실패 | 5xx · `408` · `429` · 네트워크 예외 | 백오프 후 다음 시도 | `fetch.py:202-206` |
...
기본값(`fetch.py:108-118`: `timeout=30.0`, `retries=3`, `backoff=0.8`)으로 요청 한
...
![병렬 수신에서 완료 순서를 입력 순서로 되돌리는 방법](images/08-order-restore.svg)
...
(`playlist.py:236-247`). 그래서 `cli.py:423` 이 만드는 작업 목록은 이런 모양이 된다.
...
1씩 늘고 `on_done(done, res)` 로 넘어간다(`cli.py:426-431` 의 `tick`). **진행률과 결과
...
남는다면 — 반환 리스트의 **길이가 줄어들고**, `cli.py:452` 의 `zip` 이 그 지점부터
...
![세그먼트 20건의 TTFB 를 평균 p50 p95 세 지표로 본 결과](images/08-tail-latency.svg)
...
§8.6.2 의 표본에 두 방식을 모두 적용한 결과다. 임계는 `report.py:187` 의 3000ms 다.
...
`report.py:160-230` 이 계측치를 판정으로 바꾸는 전 구간이다. 먼저 집계다.
...
`"segments"` 에서 `"failed"` 를 빼야 나오고(`report.py:221-222`), `tput` 은 처리량이 0 인
...
(`report.py:163`), **가장 오래 걸린 요청들이 정확히 그 이유로 표본에서 빠진다.**
...
`408`·`429` 를 하드코딩으로 예외 처리한 것(`fetch.py:200`)은 **그 목록이 유도되지 않고
열거될 수밖에 없는 성질**의 결과다.
...
이 코드는 `429` 를 재시도 대상에 넣어 규격을 지킨다(`fetch.py:200`). 그러나 **존중의
수준을 정직하게 평가하면 절반이다.**
...
| 역할 | 해야 할 일 |
|---|---|
| **송출 서버 운영자** | 속도 제한에 **정확한 상태 코드**를 쓴다. `429` 를 제대로 다루는 클라이언트는 동시성을 낮추고 `Retry-After` 만큼 기다리지만, `503` 을 받으면 그것을 일시 장애로 읽어 **같은 속도로 재시도한다**. 이 코드는 둘을 구별하지 않으므로(`fetch.py:200` 은 5xx 도 `429` 도 재시도 대상으로 둔다) 그 차이가 드러나지 않는다. 상태 코드를 정확히 고르는 것이 서버 자신의 방어다 |
...
- **`Retry-After` 를 읽지 않는다.** `fetch.py:196-201` 은 `e.headers` 에서
...
- **표본 수가 분위수 옆에 없다.** `ttfb_ms_p95` 를 해석하려면 표본 수가 필요한데
  (`report.py:227-229` 옆에 없고 `"segments"` 에서 `"failed"` 를 빼야 나온다), 이것이

...
> 제3부 접근 통제와 그 한계 · 9/16
> 선행: 제8장(병렬성·재시도·계측) · 후행: 제10장(CORS 헤더가 흘리는 것)
...
![Referer 를 쓰는 주체가 누구인지에 따라 갈리는 통제의 효력](images/09-enforcer-location.svg)
...
![회차 하나를 여는 요청 사슬 네 단계와 단계별 Referer](images/09-referer-chain.svg)
...
|---|---|---|---|---|
| 0 | 시리즈 목록 `/c/<제목>` | 사이트 루트 | 사이트 | `series.py:157` `series.py:160` |
| 1 | 회차 페이지 `/e/<제목> N화` | 사이트 루트 | 사이트 | `series.py:272` |
| 2 | 플레이어 `<플레이어>/video/<해시>` (iframe) | **회차 페이지 전체 주소** | 사이트 | `series.py:278` |
| 3 | `player/index.php?…&do=getVideo` (XHR·POST) | **플레이어 전체 주소** | 플레이어 | `series.py:283` |
| 4 | m3u8 · 세그먼트 (CDN) | 플레이어 origin | 플레이어 | `series.py:300` → `cli.py:907-908` |
...
이 인스턴스가 `series.discover()`(`cli.py:790`)와 `series.resolve()`
(`cli.py:884`, `cli.py:743`)를 모두 처리한다. 따라서 `self.headers.update(extra)` 로
...
갈라진 증상이 난다 — `probe.py:58-59` 가 같은 취지를 적어 둔 지점이다.
...
클라이언트의 재시도 정책까지 오염시킨다 — `fetch.py:199-201` 은 `408`·`429` 를 뺀 4xx 를
...
| **클라이언트 도구 작성자** | 실패 진단에 Referer 를 **첫 후보로** 넣는다(`cli.py:95`). 404 를 액면대로 믿게 두면 사용자가 존재하지 않는 문제를 찾는다 |
| 〃 | 단계마다 다른 값을 보내야 한다면 **요청 지역 헤더**로 구현한다(`fetch.py:146-150`). 인스턴스에 쓰면 오염이 조용히 번진다(§9.5.3) |
...
를 보내야 통과하는가"의 답이다. `cli.py:105-124` 의 `_adopt_origin` 이 정확히 그
누출을 이용한다. 제10장의 주제다.
...
- **세그먼트 요청에는 `_from` 이 관여하지 않는다.** 4단계 이후는 `Fetcher.headers` 에
  들어간 값 하나로 전부 나간다(`cli.py:907-908`). 세그먼트마다 다른 Referer 를 요구하는

...
> 제3부 접근 통제와 그 한계 · 10/16
> 선행: 제9장(핫링크 차단의 해부 — Referer 라는 자기 신고) · 후행: 제11장(서명 URL — 시간 제한 능력의 설계)
...
![ACAO 가 작용하는 지점의 비교](images/10-acao-scope.svg)
...
이 저장소의 주석은 이 구분을 정확히 적어 두었다(`cli.py:112-114`).
...
![구성값 하나가 검사와 선언을 동시에 낳는다](images/10-one-config-two-paths.svg)
...
`FetchResult` 는 요청 한 건의 계측치를 담는 자료형이고(`fetch.py:73-104`), 상태·크기·
...
`fetch.py:200-201` 은 4xx 에서 재시도 루프를 즉시 끊는다. 그런데 `cli.py:133` 은
바로 그 4xx 를 한 번 더 요청한다. 모순이 아니다 — **두 재시도는 다른 것이다.**

![두 종류의 재시도](images/10-two-retries.svg)

*그림 10-3 — 두 종류의 재시도*

| | 전송 계층 재시도 (`fetch.py:205-206`) | 상태 변경 후 재시도 (`cli.py:132-133`) |
|---|---|---|
...
- `_adopt_origin(...)` 이 `False` 인데 재시도하면 **똑같은 요청의 반복**이 되어
  `fetch.py:200` 이 이미 무의미하다고 판정한 일을 다시 한다.
...
ffmpeg, ffprobe 실측, 자막 트랙 추출(`README.md:228-233`, `probe.py:61`).
...
도구는 추측하지 않고 물러선다(`cli.py:319-322`) — 짐작으로 만든 URL 은 헛요청이
되기 때문이다.
...
| **도구 구현자** | 추정으로 얻은 값과 사용자가 확정한 값을 구분해 다룬다. 추정은 화면에 밝히고(`cli.py:123`), 사용자 지정이 언제나 이긴다(`cli.py:116`) |
...
- **채택 지점은 한 곳뿐이다.** `_adopt_origin` 은 `_load` 에서만 호출된다
  (`cli.py:132`). 마스터 플레이리스트가 200 으로 열린 뒤 **variant 요청이 403** 인
  경우(`cli.py:185-187`)나 세그먼트 병렬 수신이 막히는 경우에는 채택이 일어나지
  않는다. 후자는 `cli.py:471` 에서 "토큰 만료 또는 Referer 검증 실패 가능성"이라는
...
- **ACAO 와 요구 Referer 의 일치는 경험칙이다.** 이 장은 §10.4 에서 그 일치의
  ...
  않았다.** 코드 주석도 "어디까지나 추정"이라고만 적는다(`cli.py:113-114`). 불일치
...
4. 도구가 `Origin` 을 보내지 않았는데도 구체적 출처가 돌아온다는 사실이,
   **그 값이 반사가 아니라 서버 구성의 정적 상수**임을 증명한다(`fetch.py:115`,
   `cli.py:488-501`).

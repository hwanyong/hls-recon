...
> 제3부 접근 통제와 그 한계 · 11/16
> 선행: 제10장(CORS 헤더가 흘리는 것 — ACAO 의 오독과 정보 누출) · 후행: 제12장(자격증명의 앰비언트 권한)
...
![신원 기반 접근 통제와 능력 기반 접근 통제의 대비](images/11-capability-vs-identity.svg)
...
![같은 해시 함수로 만든 세 가지 서명 구성의 비교](images/11-mac-constructions.svg)
...
`Episode`(`series.py:60-66`)와 `Play` 가 **별개의 자료형으로 나뉘어 있다**는 것이
설계의 핵심이다.
...
한다는 제9장의 제약도 여기서 그대로 반복된다(`series.py:99-109`).

![미리 모으기와 지연 해석의 시간축 비교](images/11-late-resolution.svg)
...
|---|---|---|
| `403` · `404` | `FetchResult.ok = False`. 4xx 이므로 재시도 없이 중단 | `fetch.py:199-201` |
| `401` | 위와 같음 | 위와 같음 |
| `200` + HTML 오류 페이지 | 선두 바이트가 `<!DO…` → `sniff()` 가 `unknown` | `tsanalyze.py:20-37`, `cli.py:459-464` |
| 세그먼트가 하나도 오지 않음 | 종료 메시지가 만료 가능성을 명시 | `cli.py:471` |
| 플레이리스트 자체가 거부 | 진단 문구가 만료를 후보로 안내 | `cli.py:97-100` |

세 번째 행이 이 교재의 여러 장을 잇는다. `tsanalyze.py:23` 의 주석은 이 상황을 직접
...
판정 쪽 주석도 같은 이유를 적어 둔다(`report.py:198`).
...
(`report.py:36-46`)과 같은 문제가, 서명 URL 에서는 **URL 자체**에 대해 제기된다.
...
- **만료 값을 파싱하지 않는다.** 이 도구는 `expires` 를 읽지 않는다. 읽으면 (a) 남은
  ...
  추정이 아니라 사실로 말할 수 있다. 지금은 `cli.py:97` 처럼 **가능성으로만** 안내한다.
  구현하지 않은 개선이다.
- **만료 실패 시 자동 재해석이 없다.** `resolve()` 는 회차 시작 시점에 한 번 불린다
  (`cli.py:884`). 회차 도중 만료가 나면 그 회차는 실패로 끝난다. 재해석 후 재시도는
...
- **`--delay` 와 만료의 상호작용은 코드 순서에서 읽은 것이다.** `sleep` 이 반복의 끝에
  있다는 사실(`cli.py:933-934`)에서 나온 결론이고, 그 배치로 실제 만료 예산이 얼마나
...
1. `?md5=<서명>&expires=<unix>` 는 **능력 기반 접근 통제**다. URL 그 자체가
   권한증표이고, 소지하면 행사할 수 있다(`series.py:1-11`).
...
4. 이 저장소는 회차 단위로 지연 해석한다(`series.py:266-300`, `cli.py:884`). 자료형이
   그 결정을 담고 있고(`Episode` ↔ `Play`, `series.py:81-87`), 부수 효과로 재고가 있는
   회차에는 발급 요청이 한 건도 나가지 않는다(`cli.py:872-874`).
5. **만료는 전용 상태 코드를 갖지 않는다.** `403`·`404` 로도, `200` + HTML 오류
   페이지로도 온다. 그래서 제14장의 **선두 바이트 판별**이 만료 검출의 일부가 된다
   (`tsanalyze.py:23`, `cli.py:459-464`).
6. **재시도는 만료를 고치지 못한다.** 같은 요청을 반복하는 것이기 때문이다
   (`fetch.py:199-201`). 필요한 것은 재시도가 아니라 **재해석**이고, 이 저장소는
   그것을 자동화하지 않았다.

...
> 제3부 접근 통제와 그 한계 · 12/16
> 선행: 제11장(서명 URL — 시간 제한 능력의 설계) · 후행: 제13장(난독화는 보안이 아니다)
...
| 위치 | 통과시키는 것 | 손대는 것 |
|---|---|---|
| `_normalize_cookie` (`cli.py:39-54`) | 쿠키 값 전체 | 껍데기·줄바꿈 |
| `normalize_url` (`fetch.py:36-54`) | 이미 인코딩된 `%` | 인코딩되지 않은 비ASCII |
| `sniff` (`tsanalyze.py:20-37`) | 세그먼트 바이트 | — (판별만 하고 변형하지 않는다) |
| `concat_segments` (`assemble.py:93-105`) | 세그먼트 바이트 전체 | — (그대로 이어 붙인다) |
...
| `-headers` (ffmpeg·ffprobe 5개 호출부) | `probe.py:77` 의 문자열 결합 | 검사 없음 | **헤더가 추가되어 전송된다** |
...
> 조건으로 요청하게 되고, 그 순간 `cli.py:526-528` 이 명시한 "헤더의 단일 출처"
> 원칙이 무너진다.
...
| `_adopt_origin` 의 `Referer`/`Origin` (`cli.py:118-122`) | **그렇다** (응답 헤더 ACAO) | 아니다 — `urlparse` 가 `\r`·`\n`·`\t` 를 제거한다 |
| 시리즈 모드의 `play.referer` (`cli.py:750-751`) | **그렇다** (원격 HTML) | 아니다 — `_origin()` 이 `urlparse` 를 거친다 |
...
![붙여넣은 쿠키 한 줄이 지나가는 네 지점](images/12-credential-trail.svg)
...
| 호출부 | 무엇을 실행하는가 |
|---|---|
| `probe.py:127` `probe()` | ffprobe — 스트림 구조 실측 |
| `probe.py:239` `first_pts()` | ffprobe — 첫 PTS 측정 |
| `assemble.py:82` `remux_from_url()` | ffmpeg — `remux` 모드 재조립 |
| `subtitles.py:143` | ffmpeg — 자막 트랙 추출 |
| `subtitles.py:360` `embed_args()` | ffmpeg — 자막 내장 (트랙 수만큼 반복) |
...
> 무엇이 깨지는지도 코드에 적혀 있다. `probe.py:58-59` 가 그것이다 —
...
`subtitles.embed_args()`(`subtitles.py:357-366`)는 자막 트랙마다 `input_args()` 를
...
![편집이 닿는 범위 — 리포트 JSON 한 장](images/12-redaction-scope.svg)
...
**(가) `-i` 인자의 URL.** `assemble.remux_from_url()`(`assemble.py:81-90`)이 만드는
...
**(나) `source` 필드.** `Report.to_json()`(`report.py:95-108`)이 직렬화하는 첫 키가
`"source"` 이고, 그 값은 `_run_one` 에 넘어온 원본 주소다. 시리즈 모드에서는
`cli.py:916` 이 `play.playlist_url` — 즉 **방금 발급받은 서명 플레이리스트 주소** —
...
받는다(`cli.py:1019`). 즉 사용자가 넣을 수 있는 헤더 이름의 집합은 무한하고,
...
- **URL 안의 자격증명은 편집 범위 밖이다.** `mux_command` 의 `-i` 인자와 `source`
  필드에 서명 URL 이 평문으로 남는다(`assemble.py:83`, `report.py:98`,
  `cli.py:916`). 리포트 JSON 을 받은 제3자는 만료 전까지 같은 자원에 접근할 수 있다.
...
- **argv 노출은 현재 인터페이스로는 피할 수 없다.** `--cookie` 외에 파일·표준 입력
  경로가 없고(`cli.py:1021-1026`), 자식 프로세스 쪽은 ffmpeg 이 `-headers` 외의

...
> 제3부 접근 통제와 그 한계 · 13/16
> 선행: 제12장(자격증명의 앰비언트 권한) · 후행: 제14장(세그먼트 확장자 위장)
...
![packed JS 호출의 네 부분과 복원 절차 네 단계](images/13-unpack-anatomy.svg)
...
![같은 정규식이 두 곳에서 끊긴다](images/13-token-boundary.svg)
...
|---|---|---|---|
| 플레이어 iframe 주소 | **예외** | `series.py:263` | 없으면 이후 전부 불가능 |
| 재생 소스 응답이 JSON 이 아님 | **예외**(서버 문구를 그대로 실어) | `series.py:292-294` | 그 문구가 곧 거절 사유다 |
| 응답에 재생 주소가 없음 | **예외** | `series.py:298` | 받을 대상이 없다 |
| 플레이어 설정(packed JS) | **빈 문자열** | `series.py:236` | 이름은 대체 가능하다 |
...
`securedLink`/`videoSource` 에서 온다(`series.py:296`). `settings` 가 결정하는 것은
...
![같은 바이트열, 두 개의 경계](images/13-trust-boundary.svg)
...
- **`re.ASCII` 반례는 합성 입력으로 재현했다.** §13.3.3 의 표는 이 장에서 만든
  ...
  `series.py:306` 의 예시(`그렌라간01.mkv`)가 가리키는 상황과 같다.
...
6. **실패 시 반환값은 값의 소비자가 정한다.** 대체 경로가 있는 이름은 빈 문자열로
   물러서고(`series.py:236`), 대체 경로가 없는 재생 주소는 즉시 예외다

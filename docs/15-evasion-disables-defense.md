# 제15장 — 회피가 방어를 끄게 만들 때

**CVE-2023-6602 와 `allowed_extensions ALL`, 그리고 측정되지 않은 개선**

> 제3부 접근 통제와 그 한계 · 15/16
> 선행: 제14장(세그먼트 확장자 위장) · 후행: 제16장(콘텐츠 스니핑의 양면)
...
주석도 이 인자가 왜 필요한지를 성실히 설명한다(`probe.py:64-73`). 그러나 **이 인자가
...
![공격자가 통제하는 것은 플레이리스트 텍스트 한 장뿐이다](images/15-attack-chain.svg)
...
![allowlist 는 취약점을 고치지 않고 도달 경로를 좁힌다](images/15-mitigation.svg)
...
![결정하는 쪽과 비용을 치르는 쪽이 다르다](images/15-externality.svg)
...
![우연한 방어 — ②는 ③에 가려져 있다](images/15-incidental-defense.svg)

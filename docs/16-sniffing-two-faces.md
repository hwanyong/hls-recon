...
> 제3부 접근 통제와 그 한계 · 16/16
> 선행: 제15장(회피가 방어를 끄게 만들 때) · 후행: 제17장(MPEG-TS 패킷 해부)
...
![브라우저 경로와 이 도구의 경로에서 판별 결과가 흘러 들어가는 곳](images/16-verdict-destination.svg)
...
하며, `_MP4_BOXES`(`tsanalyze.py:17`)의 8개 집합 중 하나면 통과다.

> **용어** — **매직 넘버(magic number)**: 파일 형식을 식별하기 위해 고정 위치에 놓이는
> 바이트열. 제14장 §14.2 에서 정의했다. MPEG-TS 처럼 매직 넘버 대신 **주기 구조**로
> 판별해야 하는 포맷도 있다.

판별 결과가 흘러가는 곳은 `cli.py:459-464` 의 `bogus` 목록이고, 거기서
`report.py:198-211` 의 "페이로드 유효성" 검사 항목이 된다. **판별과 판정 사이에
...
(`library.py:17`)에 있으나 위 세 갈래에 없는 확장자 — 현재 목록에서는 `.avi` 하나 —
...
`_diagnose`(`cli.py:57-102`)는 네 갈래로 내용을 판별하지만, 그 결과로 하는 일은
...
![세 판별 구현이 파일의 어느 범위를 읽는가](images/16-scope-of-inspection.svg)
...
3. **예외를 던지지 않는다.** 판별 실패는 값으로 표현된다(`"unknown"`, `""`). 예외는
   `OSError` 처럼 판별과 무관한 실패에만 쓰인다(`inventory.py:148-149`).
...
- **같은 판별이 두 곳에 따로 구현돼 있다.** MPEG-TS 판별 규칙(`0x47` 이 188바이트 주기)
  이 `tsanalyze.sniff`(`tsanalyze.py:31-34`)와 `inventory._ts_flaw`(`inventory.py:113-121`)
...
- **`.avi` 는 구조 검사를 받지 않는다.** §16.3.4 의 `else: why = ""` 분기다. `MEDIA_EXTS`
  (`library.py:17`)에 있으나 세 갈래 어디에도 없는 유일한 확장자이며, 64 KB 크기 검사만
...
- **`_sniff_format` 의 전량 재검사에는 크기 상한이 없다.** 선두 4 KB 에서 큐를 찾지
  못하면 본문 전체를 UTF-8 로 디코드한다(`subtitles.py:425-427`). `body` 는 이미 메모리에
...
7. **판별 결과가 저장되어 다른 결정의 입력이 되면 위험은 소비 시점에 정해진다.**
   이 저장소에서 판별값이 파일 확장자가 되는 자리(`subtitles.py:475`)가 안전한 이유는
   **치역이 닫힌 집합**이기 때문이다.

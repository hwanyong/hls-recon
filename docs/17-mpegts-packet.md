...
> 제4부 비트 수준 · 17/22
> 선행: 제16장(콘텐츠 스니핑의 양면) · 후행: 제18장(4비트 순환 카운터의 한계)
...
|---|---|---|
| **재동기가 계산 가능** | 다음 패킷의 시작 위치는 언제나 `현재 + 188`. 탐색이 아니라 산술 | `analyze` 의 루프 보폭(`tsanalyze.py:85`) |
| **판별이 O(1)** | 두 바이트(`data[0]`·`data[188]`)만 읽으면 컨테이너를 안다 | `sniff`(`tsanalyze.py:31-34`), `_ts_flaw`(`inventory.py:113-121`) |
...
![TS 패킷 헤더 4바이트를 32비트로 펼친 지도](images/17-header-bits.svg)
...
![188바이트의 몸통 배치를 AFC 2비트가 정한다](images/17-packet-layout.svg)
...
`pids` 배열에 8191 이 섞인다(`report.py:250-256`). 검증 결과를 놓고 "이 송출은 트랙이
...
(`tsanalyze.py:117`). **판정에 필요한 것은 개수이고, 상세는 진단용 표본이다.** 둘을
...
## 17.7 같은 판별의 축소판 — `inventory.py:113-121`
...
`inventory.py:17-19` 가 그 판단의 근거를 적어 두었다.
...
엄격하면 멀쩡한 27화를 매번 다시 받는다(`inventory.py:13-15`). 그래서 이 자리에서는
...
마지막으로 이 축소판의 한계다. **`.ts` 확장자에만 적용된다**(`inventory.py:144-145`).
...
| 방어 코드 | 없어도 된다 | `box_size < 8`·`offset + box_size > size` 검사 필수(`inventory.py:92-95`) |
...
| **프레임 단위 암호화의 일부 구현** | 세그먼트 통째 복호가 성립하지 않는 방식. `EXT-X-KEY` 단계에서 이미 거부된다 | `playlist.py:62-64` 가 앞에서 차단 |
...
기록된다(`cli.py:459-464`, 제14장 §14.4.2). TSC 검사가 잡는 것은 그와 다른 상황 —
...
![스크램블된 패킷에서도 헤더 4바이트는 평문으로 남는다](images/17-clear-header.svg)
...
이것이 `report.py:245` 가 미복호를 WARN 이 아니라 **FAIL** 로 두는 이유다. 파일은
...
1. `playlist.py:62-64` — `METHOD` 가 `AES-128`·`NONE` 이 아니거나 `KEYFORMAT` 이
   `identity` 가 아니면 지원 대상에서 제외
2. `decrypt.py:36-40` — 그럼에도 세그먼트 단위 복호를 시도하면 `NotImplementedError`
3. `cli.py:459-464` — 복호 결과가 미디어가 아니면 페이로드 유효성 FAIL
...
- **`analyze()` 단독으로는 TS 임을 보증하지 않는다.** 실측: `0x47` 로 시작하는 201바이트
  ...
  가 먼저 걸러 주므로 문제되지 않지만(`cli.py:459-465`), **함수 단독으로 쓰면 보증이

...
> 제4부 비트 수준: 컨테이너와 무결성 · 18/22
> 선행: 제17장(MPEG-TS 패킷 해부) · 후행: 제19장(자기동기 포맷과 연결의 대수)
...
![MPEG-TS 패킷 헤더 4바이트의 비트 배치와 continuity counter 의 위치](images/18-cc-field.svg)
...
> 리포트 문구(`report.py:236`)가 `CC 불연속 5건(패킷 유실)` 이므로 오독하기 쉽다.
...
![유실 패킷 수 k 를 16으로 나눈 나머지 중 정확히 둘이 검사를 빠져나간다](images/18-blind-residues.svg)
...
평균이 이상치를 감추는 것과 같은 문제가 여기서도 나타난다(`report.py:56-61` 이
...
FAIL 이다(`report.py:245`). 두 검사의 실패 모형이 겹치지 않게 설계되어 있다는 뜻이다 —
...
**페이로드 유효성 FAIL** 로 승격시킨다(`cli.py:459-464`). 즉 이 구멍은 상류에서 막힌다.
...
![상태를 세그먼트 경계 너머로 잇는가에 따라 같은 결손이 다른 결론을 낳는다](images/18-state-across-segments.svg)
...
보지 않는다.** 파서는 그 태그를 읽고 있고(`playlist.py:331`, `Segment.discontinuity` —
`playlist.py:145`), 타임라인 검사는 그것을 참작한다.
...
**그러나 TS 무결성 검사에는 같은 참작이 없다.** `cli.py:465` 는 세그먼트의
...
- **`EXT-X-DISCONTINUITY` 를 CC 검사가 참작하지 않는다.** 파서는 그 태그를 읽고
  (`playlist.py:331`) 타임라인 검사는 참작하지만(`report.py:309-310`),
  `cli.py:465` 는 세그먼트의 `discontinuity` 플래그와 무관하게 같은 `cc_state` 를
...
- **`cc_detail` 이 20건에서 잘린다**(`tsanalyze.py:117`, `tsanalyze.py:68`). 대량 유실
...
- **fMP4 세그먼트에는 이 검사가 없다.** `analyze()` 는 선두가 `0x47` 이 아니면 즉시
  돌아온다(`tsanalyze.py:78-80`). ISO-BMFF 세그먼트의 전송 무결성은 다른 방법으로

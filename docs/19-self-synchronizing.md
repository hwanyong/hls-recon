# 제19장 — 자기동기 포맷과 연결의 대수

**왜 바이트를 그냥 이어붙여도 되는가**

> 제4부 비트 수준 · 19/22
> 선행: 제18장(4비트 순환 카운터의 한계) · 후행: 제20장(ISO-BMFF — 재귀 TLV 구조)
...
> `SYNC_BYTE = 0x47`(`tsanalyze.py:12-13`).
...
![유효한 TS 둘을 이어붙이면 다시 유효한 TS 다](images/19-concat-closure.svg)
...
> data) 박스로 이루어진다. 이 저장소는 확장자 `.m4s` 로 다룬다(`cli.py:450`).

fMP4 세그먼트에는 동기 바이트가 없다. 188주기의 `0x47` 대신 박스 구조(TLV, 제20장)로
프레이밍되며, `sniff` 는 이것을 오프셋 4–8 의 박스 타입으로 판별한다(`tsanalyze.py:16-17`
...
`paths.append(ipath)` 가 세그먼트 루프(`cli.py:452-468`)보다 먼저 실행되므로, `init.mp4`
는 `paths` 의 0번 원소가 되고 `concat_segments` 가 이것을 가장 먼저 기록한다. **코드가
init 을 맨 앞에 두는 이유가 곧 §19.4.2 의 닫힘 회복 조건이다.**

![init 세그먼트를 맨 앞에 두면 fMP4 도 연결로 합쳐진다](images/19-fmp4-init-prefix.svg)
...
이유가 이것이다(`cli.py:444-445`). init 이 없으면 나머지를 아무리 잘 받아도 결과가
...
기본 경로)에는 `+genpts` 가 없다(`assemble.py:81-88`). 그 경로에서는 ffmpeg 의 hls
...
경계의 합이 파일 크기와 맞는지로 완결성을 검사하는 `inventory.py:67-102` 를 읽으며,

...
> 제4부 비트 수준: 컨테이너와 무결성 · 21/22
> 선행: 제20장(ISO-BMFF — 재귀 TLV 구조) · 후행: 제22장(임계값 설계)
...
![90kHz 눈금의 유래](images/21-clock-90khz.svg)
...
![PES 헤더의 PTS 5바이트 배치](images/21-pts-bitfield.svg)
...
![결손이 표현 변환을 살아남는 경로](images/21-hole-survives-remux.svg)
...
|---|---|---|
| 대상 | **재조립된 로컬 산출물** (`cli.py:616`) | **원격 세그먼트 원본** (`cli.py:266`) |
...
계층에서는 4비트 순환 카운터의 차분을 보고(`tsanalyze.py:112-119`), 표시 시각 계층에서는
...
**경계 결손은 총량에만 나타난다.** `report.py:262-278` 의 길이 정합 검사는 갭
...
- **`ok=False` 가 리포트에 나타나지 않는다.** `report.py:304` 의 조건은
...
- **불연속 완화는 위치를 대조하지 않는다**(§21.6.2). 개수만 비교하므로 "선언된 이음매
  ...
  있으므로 고칠 수 있는 미탐이다.
- **갭 스캔은 산출물에만 돈다**(`cli.py:616`). 원본 세그먼트에는 돌지 않으므로

...
> 제4부 비트 수준: 컨테이너와 무결성 · 22/22
> 선행: 제21장(PTS 와 90kHz — 왜 총 길이로는 결손을 못 잡는가) · 후행: 제23장(AES-128-CBC 와 IV 유도 규칙)
...
6초가 통째로 없어졌는데 길이가 같다. `길이 정합` 검사(`report.py:263-278`)는 두 파일에
...
![임계 400ms 선 하나가 네 경우를 가른다](images/22-threshold-line.svg)
...
이 저장소는 그 전환을 코드로 인정해 두었다 — `--no-gap-scan`(`cli.py:1067`). §22.5.4 에서
그 설계의 값과 대가를 따로 본다.
...
![갭 판정 여섯 단계와 각 단계를 빼면 생기는 오류](images/22-gap-pipeline.svg)
...
담기지 않으므로, `a > b` 인 항목이나 `length` 프로퍼티(`probe.py:172-174`)의 음수 값은
...
§22.3.2 에 인용한 `report.py:319-324` 는 PASS 를 낼 때도 `간격 중앙값` 과 `임계` 를 함께
찍는다. 이것은 장식이 아니다.
...
같은 값이 JSON 에도 남는다(`report.py:325-335`, `threshold_sec` · `frame_interval_sec`).
...
있고(`report.py:123-511`), 그 안의 상수를 전수로 훑으면 **근거의 등급이 뚜렷하게 갈린다.**
...
두었다 — "한 세그먼트 이상 어긋났으면 구간 결손으로 본다"(`report.py:266`). 즉 임계가
...
건너뛴다. 기본값이 `0.0` 이고(`playlist.py:156`) `#EXT-X-TARGETDURATION` 줄에서만 채워지므로
(`playlist.py:298`), 그 태그가 없는 비표준 플레이리스트에서는 **강한 판정이 조용히
...
| 검사 | 앵커 | 임계 | 1건일 때 |
|---|---|---|---|
| 세그먼트 수신 실패 | `report.py:167-172` | 없음 | FAIL |
| 페이로드 유효성(선두 바이트) | `report.py:199-206` | 없음 | FAIL |
| CC 불연속 | `report.py:235-236` | 없음 | WARN |
| 세그먼트 해시 중복 | `report.py:213-218` | 없음 | WARN |
| 전체 디코드 오류 | `report.py:502-508` | 없음 | FAIL |
...
| 드리프트 FAIL | `report.py:267` | 상대(TARGETDURATION) | 유도 가능 — 세그먼트 1개분 | FAIL |
| 드리프트 WARN 0.5% | `report.py:269` | 상대 비율 | **없음** | WARN |
| TTFB p95 3000ms | `report.py:187` | **절대** | **없음** | WARN |
| 자막 이탈 +5.0s / −0.5s | `report.py:360,432` | **절대** | **없음** | **FAIL** |
| 자막 커버리지 20% | `report.py:457` | 절대 비율 | **없음** | WARN |
...
보고된다.** `discontinuities` 가 0 이므로 `intended` 가 거짓이 되어(`report.py:309-310`)
...
|---|---|---|
| `EXT-X-DISCONTINUITY` 선언 수 | 편집 이음매만 설명한다 — VFR 은 설명 못 함 | **일부 사용** (`report.py:309-310`) |
| 세그먼트 수신 실패 기록 | 전송 중 유실이면 가른다 | 사용 (`report.py:167-172`) |
| MPEG-TS continuity counter | 패킷 유실이면 가른다 (16배수 미탐, 제18장) | 사용 (`report.py:235-236`) |
...
(`report.py:304`)이 통째로 건너뛰어져 **`타임라인 연속성` 항목 자체가 사라진다.** JSON 의
`stats["timeline"]` 도 없다. 같은 파일 안에서 `--limit` 표본 실행은 자막 타임라인 검사에
대해 명시적으로 "판정 보류"를 출력하는데(`report.py:424-430`), 갭 스캔은 침묵한다. **"검사
...
|---|---|---|---|
| HTTP 요청 실패 | 세그먼트 수신 (`report.py:167-172`) | 없음 | 매우 높음 — 이산 사건 |
| 200 응답인데 미디어가 아님 | 페이로드 유효성 (`report.py:199-206`) | 없음 | 매우 높음 — 선두 바이트 |
| 전송 중 TS 패킷 유실 | CC 검사 (`report.py:235-236`) | 없음 | 중간 — 정확히 16의 배수면 미탐(제18장) |
| 재조립 결과의 총 길이 이상 | 길이 정합 (`report.py:267`) | TARGETDURATION | 중간 — 총량만 본다 |

# 제23장 — AES-128-CBC 와 IV 유도 규칙

**media sequence 를 IV 로 쓴다는 것의 의미**

> 제5부 암호: 보호와 보호처럼 보이는 것 · 23/26
> 선행: 제22장(임계값 설계) · 후행: 제24장(패딩 오라클의 반대편)
...
검사가 비로소 의미를 갖는다는 뜻이고, 그래서 `cli.py:456-458` 의 순서는 협상 대상이
아니다.
...
![CBC 복호화의 연쇄와 IV 의 자리](images/23-cbc-chain.svg)
...
![RFC 8216 의 IV 유도 두 갈래](images/23-iv-derivation.svg)
...
`seq` 는 파서 지역 변수로 0 에서 시작하고(`playlist.py:220`), `EXT-X-MEDIA-SEQUENCE` 를
...
같은 성질이 파싱 쪽에도 있다. `playlist.py:314` 는 IV 문자열이 `0x` 로 시작하지 않으면
...
![평문 주입 경로의 유무가 결론을 가른다](images/23-injection-path.svg)
...
**(3) 키 요청은 세그먼트와 같은 `Fetcher` 를 쓴다**(`decrypt.py:25`). 따라서 사용자가
...
- **이 저장소의 회귀 테스트는 기본 IV 경로를 지나가지 않는다.** ffmpeg 8.1.1 의 HLS
  ...
  주입 8종에 없는 항목이다.
- **IV 문법 오류가 조용한 오염이 된다.** `playlist.py:314` 는 `0x` 접두가 없는 IV 를
...
- **IV 길이가 16바이트가 아닌 입력에 대한 진단이 없다.** `KeyCache.material` 은 키
  ...
  수준 예외가 그대로 사용자에게 간다. `cli.py:57-102` 의 `_diagnose` 같은 층이 암호
  경로에는 없다.
- **`EXT-X-MAP` 은 복호화되지 않는다.** `cli.py:441-448` 은 초기화 섹션을 받은 바이트

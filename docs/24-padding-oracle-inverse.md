...
> 제5부 암호 · 24/26
> 선행: 제23장(AES-128-CBC 와 IV 유도 규칙) · 후행: 제25장(AES-128 은 DRM 이 아니다)
...
![PKCS#7 마지막 블록의 세 가지 모양](images/24-pkcs7-block.svg)
...
| 지점 | 코드 | 예외를 잡는가 |
|---|---|---|
| `cli.py:458` | `data = keys.decrypt(...)` | 아니오 |
| `cli.py:587` | `run = _run_segments(...)` | 아니오 |
| `cli.py:678` | `rep = _run_one(...)` (단건 실행) | 아니오 |
| `cli.py:916` | `rep = _run_one(...)` (시리즈 일괄 실행) | **`SystemExit` 만** 잡는다 |
| `cli.py:1117` `main()` | — | 전역 처리기 없음 |

이 저장소는 "사용자에게 보일 실패"를 `SystemExit` 으로 통일해 던진다(`cli.py:135`
`:143` `:445` `:471` 등). 시리즈 일괄 실행은 그 규약에 기대어 **한 회차의 실패를
삼키고 다음 회차로 넘어간다**(`cli.py:917-922`).
...
![손상 하나가 어디에서 이름을 얻는가](images/24-diagnosis-path.svg)

*그림 24-2 — 손상 하나가 어디에서 이름을 얻는가*

오른쪽 경로에서 손상을 잡는 것은 두 장치다.

**첫째, 컨테이너 판별**(`tsanalyze.py:20-37` `sniff`). 제5·14·16장에서 세 번 만난
그 함수다. 복호문의 선두가 `0x47` 이고 188바이트 뒤에도 `0x47` 이면 MPEG-TS,
아니면 `unknown` 이다. `unknown` 은 `bogus` 목록에 쌓이고 리포트에서 **페이로드
유효성 FAIL** 이 된다(`report.py:198-211`).

**둘째, TS 연속성 검사**(`tsanalyze.py:71-121` `analyze`). 여기서 결정적인 것은
`cli.py:438` 에서 만든 `cc_state` 딕셔너리를 **루프 밖에서 만들어 매 세그먼트에
...
있으면 WARN 이다.** 그리고 `_exit_code`(`cli.py:651-652`)는 WARN 을 0 으로 매핑한다.
...
이 예외는 `_unpad_pkcs7` 이 아니라 그 **한 줄 위**, `decrypt.py:46` 의
...
`playlist.py:48-64` 의 `Key` 데이터클래스가 이 표를 그대로 반영한다. 필드가 넷이고
다섯째가 없다.
...
- **블록 정렬 예외는 문서화되지 않은 구멍이다**(§24.3.4). 절단 길이가 16의 배수가
  아니면 `decrypt.py:46` 에서 `ValueError` 가 나고 아무도 잡지 않는다. 주석이 명시한
...
- **CC 불연속 단독은 WARN 이고 종료 코드는 0 이다**(`report.py:245`,
  `cli.py:651-652`). §24.3.3 의 "끝 1,024바이트 절단" 사례는 리포트에 기록되지만

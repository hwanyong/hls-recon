# 제25장 — AES-128 은 DRM 이 아니다

**위협 모델과 Kerckhoffs 원리**

> 제5부 암호: 보호와 보호처럼 보이는 것 · 25/26
> 선행: 제24장(패딩 오라클의 반대편) · 후행: 제26장(암호화 입자가 구조를 결정한다)
...
![키 배포 경계 — 무엇이 배제되고 무엇이 배제되지 않는가](images/25-key-distribution-boundary.svg)
...
복호화 진입점에서 한 번 더(`decrypt.py:36-40`). 두 번째는 첫 번째가 우회됐을 때
...
(`cli.py:436` 에서 `KeyCache(fetcher)` 로 주입된다). 여기서 구조적 사실 하나가 따라
...
(`decrypt.py:50-58`). 그렇다면 무엇으로 성공을 판정하는가.
...
로 올린다(`report.py:241-245`). 세그먼트 단위 복호화가 끝난 뒤에도 이 카운터가 0 이
...
![달라지는 것은 암호가 아니라 신뢰 가정의 위치다](images/25-trust-boundary-location.svg)
...
| 둘째 — "DRM 은 코드 레벨에서 거부" | **기술적 사실** | `playlist.py:64` 가 강제한다 |
...
- **키 회전의 실제 효과는 이 저장소에서 검증되지 않았다.** 파서는 중간의 `EXT-X-KEY`
  재선언을 처리하지만(`playlist.py:307-316`), 회전이 있는 스트림은 회귀 테스트에 없다.
...
6. 이 저장소는 그 경계를 **코드로** 긋는다 — `is_supported` 의 `keyformat == "identity"`
   ...
   파싱 시점에 거부된다(`playlist.py:64`, `cli.py:395-398`, `decrypt.py:36-40`).

# 제26장 — 암호화 입자가 구조를 결정한다

**SAMPLE-AES 가 거부되는 이유**

> 제5부 암호: 보호와 보호처럼 보이는 것 · 26/26
> 선행: 제25장(AES-128 은 DRM 이 아니다) · 후행: 제27장(두 시간축의 아핀 대응)
...
`METHOD` 가 가질 수 있는 값은 RFC 8216 기준으로 셋이다(`playlist.py:52` 의 주석이
그대로 열거한다).
...
`playlist.py:63` 의 주석 한 줄이 이 장 전체의 요약이다. 이 장은 그 한 줄을 펼친다.
...
![AES-128 과 SAMPLE-AES 에서 암호문이 차지하는 자리의 대비](images/26-granularity-map.svg)
...
![AES-128 과 SAMPLE-AES 에서 암호 계층이 알아야 하는 범위의 대비](images/26-layer-coupling.svg)
...
`is_supported` 는 `Key` 객체의 속성이므로, 플레이리스트를 파싱하는 순간(`playlist.py:307-316`
...
`--mode segments` 를 주면 강등하지 않는다. 그 경우 `decrypt.py:36-40` 의
...
(`cli.py:1066` 은 끄는 쪽에만 플래그를 두었다 — 기본은 켬).
...
표면은 `cli.py:57-102` 의 `_diagnose` 가 만들어 온 진단 형식과 어긋난다(§26.8).
...
| `playlist.py:57-64` | **선언값 자체** | 바뀌지 않는다 |
...
- **강제 `segments` 경로의 표면이 거칠다.** §26.5.4 의 `NotImplementedError` 는 CLI
  ...
  않지만, `cli.py:57-102` 의 `_diagnose` 가 세워 온 진단 형식과 어긋난다.
- **리포트가 암호화 방식을 잘못 표기한다.** 콘솔 구조 출력은 실제 `METHOD` 를 그대로
  보여 주지만(`cli.py:200-207`), 리포트로 넘어가는 값은 불리언 하나다.
...
5. 이 저장소는 그 결합을 감당하지 않기로 하고 **선언 단계**(`playlist.py:57-64`)와
   **복호 단계**(`decrypt.py:33-47`) 두 겹으로 거부한 뒤, `auto` 에서 `remux` 로
   강등한다(`cli.py:391-402`). 강등의 대가로 **검사가 10개에서 5개로 준다** — 제17장의
   마지막 그물까지 걷힌다.

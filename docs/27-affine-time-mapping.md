...
**X-TIMESTAMP-MAP 과 오프셋 식의 유도**

> 제6부 시간과 분산 · 27/30
> 선행: 제26장(암호화 입자가 구조를 결정한다) · 후행: 제28장(33비트 래핑과 신뢰할 수 없는 입력)
...
코드가 계산하는 식과 정확히 같다(`subtitles.py:218`). 항을 정리하면 이렇다.
...
![세 개의 원점과 구간 뺄셈으로 본 오프셋 식](images/27-three-origins.svg)
...
넣지 않고(`cli.py:284-285`), 표에 없는 트랙은 나중에 `0.0` 으로 조회된다 —
...
다시 받지 않고도 정렬을 맞출 수 있는 근거가 여기다(`cli.py:714-717`).
...
`_subtitle_offsets`(`cli.py:237-289`)의 반환형은 **트랙 URI → 오프셋(초)** 의 사전이다.
...
`embed_args`(`subtitles.py:339-374`)가 인자를 `-i` **앞에** 놓는 것도 필수다. ffmpeg 의
입력 옵션은 위치에 의미가 있어서, 뒤에 놓으면 출력 옵션으로 해석되거나 무시된다.

### 27.3.8 하나의 계산, 두 개의 적용 지점

![하나의 오프셋 계산을 두 적용 경로가 공유하는 구조](images/27-one-offset-two-paths.svg)
...
| 방어 | 위치 | 막는 것 |
|---|---|---|
| 음수 `MPEGTS` 거부 | `subtitles.py:208-210` | 부호 있는 값으로 해석되게 만드는 입력. 규격상 33비트 부호 없는 값이므로 음수는 무효다 |
| 정수 파싱 실패 시 `None` | `subtitles.py:205-207` | 정수로 읽히지 않는 값. 다만 정규식이 `-?\d+` 만 잡아 오므로 이 분기는 현재 도달하지 않는다 — `MPEGTS:1e9` 는 `1` 로 잘려 매칭된다(실측) |
| 헤더 없으면 `None` | `subtitles.py:200-202` | 근거 없는 추정. 보정하지 않는 쪽으로 퇴화한다 |
| 결과의 교차 검증 | `report.py:431-440` | **적용된 오프셋이 타당한지**를 영상 길이와 대조 |
| 기준선을 실측이 아닌 선언값으로 | `report.py:339-342` | 밀린 자막이 스스로 기준을 끌고 가는 자기 참조(제38장) |
...
- **sidecar** — `shift` 의 `total = max(0.0, total)`(`subtitles.py:227`)가 음수 시각을
...
(`report.py:375`, `report.py:492`). 이 값에는 `video_pts0` 가 들어 있고, 그것은
...
- **대응점을 첫 조각에서만 읽는다.** `cli.py:280` 은 자막 플레이리스트의 첫 세그먼트만
...
- **두 경로가 공유하는 것은 오프셋뿐이다.** sidecar 경로만 `dedupe` 를 거치므로 내장
  ...
  진술은 **정렬에 한해** 참이다.
- **1밀리초 불감대가 두 곳에 각각 적혀 있다.** `subtitles.py:223` 과
  `subtitles.py:364` 에 같은 `0.001` 이 리터럴로 들어 있다. 지금은 일치하지만 한쪽만

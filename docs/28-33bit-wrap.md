# 제28장 — 33비트 래핑과 신뢰할 수 없는 입력

**범위 검사가 없으면 계산은 조용히 틀린다**

> 제6부 시간과 분산 · 28/30
> 선행: 제27장(두 시간축의 아핀 대응 — X-TIMESTAMP-MAP) · 후행: 제29장(at-least-once 와 멱등성)
...
| `video_pts0` | 영상 첫 세그먼트를 받아 `ffprobe` 로 읽은 값(`probe.py:236-255`) | 로컬 실측 |

셋 중 둘이 서버가 주는 문자열이다. 그리고 이 식의 결과는 곧바로 파일을 고치는 데
쓰인다 — `shift()` 가 자막 파일의 **모든 큐 시각을 그만큼 이동**시킨다
(`subtitles.py:221-245`). 즉 **원격에서 온 숫자 하나가 자막 전체의 위치를 정한다.**
...
(`subtitles.py:159`), `shift()` 는 `abs(seconds) < 0.001` 에서 즉시 반환한다
(`subtitles.py:223-224`). **한 글자도 옮기지 않는다.**
...
영상 범위를 벗어나면 `report.py:431-451` 이 FAIL 을 낸다. 반면 위 표의 −60초
...
![MPEGTS 값의 세 구간과 관문](images/28-half-range-check.svg)
...
호출부(`cli.py:283`)에도, `main()`(`cli.py:1117-1128`)에도 이 예외를 받는 자리가
...
![33비트 클럭 위에서 뺄셈이 되감김을 보지 못한다](images/28-wrap-difference.svg)
...
|---|---|---|---|---|
| A (음수 방향) | −95,439.5초 | **모든 큐가 `00:00:00.000`** | **WARN** — "최소 커버리지 0%" | `report.py:452-460` |
| B (양수 방향) | +95,439.5초 | 95,449.5 ~ 95,463.5초 | **FAIL** — "영상 범위를 벗어난 트랙" | `report.py:431-451` |
...
여기에 하나 더 붙는다. `report.py:432` 의 `r.first_cue < -0.5` 조건은 별도 파일
경로에서는 성립할 수 없다 — `shift()` 가 음수 시각을 쓰지 않고, 큐 시각 정규식
(`subtitles.py:27-29`)도 부호를 읽지 않기 때문이다. 그 조건이 실제로 일하는 곳은
컨테이너 내장 경로(`report.py:358-366`)이며, 그쪽은 시각을 `ffprobe` 로 다시 읽으므로
...
| 이 코드 (`subtitles.py:209-210`) | 같은 규격 | `None` → 보정을 건너뛴다 |
...
`report.py:492`, 컨테이너 내장 경로는 `report.py:375` 이다. **도구가 파일을 고쳤다면
...
3. `subtitles.py:209-210` 이 음수를 만나면 `None` 을 돌려 **보정을 포기**한다.

# 제29장 — at-least-once 와 멱등성

**경계 중복 큐, 그리고 정규화와 비교의 순서**

> 제6부 시간과 분산 · 29/30
> 선행: 제28장(33비트 래핑과 신뢰할 수 없는 입력) · 후행: 제30장(위임의 경계)
...
![자막 조각 다섯 개와 원본 큐 여섯 개를 같은 시간축에 놓은 그림](images/29-boundary-cues.svg)
...
조각을 받아 이어붙이는 일은 ffmpeg 에 맡긴다(`subtitles.py:142-150`). 그 결과를
...
| "이미 본 키" 저장소 | `seen: set[...]` | `subtitles.py:276` |
...
주석 두 줄(`subtitles.py:296-297`)이 이 장의 명제를 코드 안에 이미 적어 두었다.
**정제가 판정보다 먼저다.**
...
![같은 큐의 두 사본을 두고 처리 순서를 비교한 그림](images/29-order-dependency.svg)
...
(`subtitles.py:288-294`). WebVTT 의 `NOTE`(주석)·`STYLE`(스타일 규칙)·`REGION`(영역
정의) 블록이 여기 해당한다.
...
제거"(`report.py:401-404`)가 실제로 일어난 일과 일대일로 대응한다.
...
`subtitles.extract` → `dedupe` 로 들어간다(`cli.py:611`, `cli.py:765`).
...
(`subtitles.py:298-301`)이 그 형태다. 정제를 별도 단계로 떼어 호출자에게 맡겼다면,
...
(`subtitles.py:269`), 블록 전부를 리스트로 만들고, `seen` 집합에 **모든 고유 큐의
...
| **이 도구 구현자** | 정규화와 판정의 순서를 **코드 주석에 남기고**(`subtitles.py:296-297`), 순서가 뒤집혔을 때만 깨지는 검사를 회귀 테스트에 둔다. 주석만 있고 테스트가 없으면 다음 리팩터링에서 순서가 바뀐다 |
...
- **SubRip 에서는 큐가 아닌 블록이 버려진다(실측).** `fmt == "srt"` 재작성 분기는
  `preserved` 를 내보내지 않는다(`subtitles.py:318-320`). SubRip 에 `NOTE` 개념이

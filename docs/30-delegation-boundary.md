...
**라이브러리가 하지 않는 일을 아는 것**

> 제6부 시간과 분산 · 30/30
> 선행: 제29장(at-least-once 와 멱등성 — 경계 중복 큐) · 후행: 제31장(유니코드 정규화)
...
![자막이 60초 밀린 실행에서 각 층이 내는 신호](images/30-silent-pass.svg)
...
![위임이 넘겨주는 세 가지](images/30-three-regions.svg)
...
| 맡긴 일 | 맡긴 이유 | 코드 앵커 |
|---|---|---|
| 세그먼트 병합(원격 경로) | hls demuxer 가 규격대로 구현 | `assemble.py:69-90` `remux_from_url` |
| 컨테이너 먹싱·변환 | 박스·PES 재작성을 다시 만들 이유가 없다 | `assemble.py:108-130` `remux_local` |
| 타임스탬프 정규화 | PTS/DTS 재기록은 컨테이너 지식이 필요 | `assemble.py:121` `-fflags +genpts` |
| AES-128 복호화(원격 경로) | `remux` 모드에서는 ffmpeg 가 키를 직접 받는다 | `assemble.py:78` 독스트링 |
| 리다이렉션·세그먼트 재시도(원격 경로) | 위와 같다 | `assemble.py:78` |
| 자막 조각 이어붙이기 | 조각 순서·형식 파싱을 다시 만들 이유가 없다 | `subtitles.py:142-149` |
| 자막 형식 변환(vtt↔srt) | 코덱 변환은 ffmpeg 의 일 | `subtitles.py:432-441` `_convert` |
| 컨테이너 자막 내장 | 매핑·메타데이터 먹싱 | `subtitles.py:339-374` `embed_args` |
| 스트림 정보·패킷 시각 실측 | ffprobe 가 컨테이너를 읽는다 | `probe.py:124-162`, `191-233` |
...
> (`-c copy`, `assemble.py:16`).

그리고 마지막 줄 `return cmd` 가 위임 설계의 마감이다. 실행한 명령을 그대로 돌려주어
리포트에 남긴다(`cli.py:637` `mux_cmd=run.mux_cmd`). **위임한 일은 재현 가능해야
...
|---|---|---|
| `X-TIMESTAMP-MAP` 정렬 보정 | ffmpeg 은 어느 입력 구성에서도 적용하지 않는다(§30.2.4) | `subtitles.py:191-218`, `cli.py:237-289` |
| 경계 중복 큐 제거 | ffmpeg 은 이어붙이기만 한다 — 규격이 허용한 중복이므로 오류도 아니다 | `subtitles.py:259-322` `dedupe` |
| 조각 헤더 정제 | 이어붙이기의 **부산물**이라 수임자에게는 정상 출력이다 | `subtitles.py:172-181` `_clean_body` |
| 전송 계층 계측 | ffmpeg 은 HTTP 상태·TTFB·회선 바이트를 보고하지 않는다(§30.7) | `fetch.py:73-104`, `report.py:159-230` |
| 페이로드 판별 | 헤더가 아니라 선두 바이트로 봐야 한다(제14·16장) | `tsanalyze.py:20-37` `sniff` |
| MPEG-TS 연속성 카운터 검사 | demuxer 는 유실을 복구할 뿐 보고하지 않는다(제18장) | `tsanalyze.py:71-121` |
| 타임라인 결손 스캔 | 총 길이가 보존되므로 ffmpeg 에게는 정상이다(제21장) | `probe.py:191-233` `gap_scan` |
...
경계 중복과 헤더 흡수 쪽은 `subtitles.py:172-181` 의 주석이 이미 기록해 둔 현상이다.
...
| 호출자 | 앵커 | 무엇을 여는가 |
|---|---|---|
| `assemble.remux_from_url` | `assemble.py:82` | 플레이리스트 URL |
| `probe.probe` | `probe.py:127` | 산출물 또는 URL |
| `probe.first_pts` | `probe.py:239` | 첫 세그먼트 |
| `subtitles.extract` | `subtitles.py:143` | 자막 플레이리스트 |
| `subtitles.embed_args` | `subtitles.py:360` | 자막 플레이리스트(내장용) |
...
표준 오류는 **실패했을 때만** 읽는다(`assemble.py:56-58`).
...
| 모듈 | 경계 선언 |
|---|---|
| `assemble.py:1-6` | "컨테이너 작업은 전부 ffmpeg 에 위임한다 … 이 모듈의 책임은 어떤 인자로 위임할지와 어떻게 계측할지 뿐" |
| `subtitles.py:1-11` | "병합은 ffmpeg 에 위임하고 여기서는 '무엇을 받을지'와 '받은 결과가 영상과 맞는지'만 책임진다" |
| `probe.py:1-4` | "플레이리스트가 선언한 값과 실제 미디어의 값을 대조하기 위한 실측 계층" |
...
- **이중 적용을 감지하는 검사가 없다.** §30.9.2 둘째 행의 위험은 실재한다.
  `timestamp_offset` 은 자막 조각의 헤더와 영상 첫 PTS 만 보고 오프셋을 계산하므로(`subtitles.py:191-218`),
  **ffmpeg 이 매핑을 적용하기 시작해도 같은 값을 낸다.** 그러면 보정이 두 번 걸린다.
  사후 검사(`report.py:421-460` 자막 타임라인)가 "영상 범위를 벗어남"으로 잡을

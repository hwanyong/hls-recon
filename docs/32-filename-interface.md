...
> 제7부 표현과 이식성 · 32/33
> 선행: 제31장(유니코드 정규화 — NFC/NFD 와 정규화 공격) · 후행: 제33장(정규식의 정확도가 곧 오분류율)
...
| 플레이어 설정 JS 안의 `title` 값 | `series.py:310-316` | 원격 서버 |
| `-o/--output` 으로 받은 파일 경로 | `cli.py:662-665` | 사람 |
...
![같은 문자열을 읽는 쪽이 셋이고 셋의 문법이 서로를 모른다](images/32-three-roles.svg)
...
실측**이다(`naming.py:24-25`). 이 장을 쓰면서 POSIX 층에서 `a:b` 가 만들어진다는 것은
...
자리는 일곱뿐이다 — `library.py:23`, `library.py:33`, `library.py:77`,
`series.py:194,314,316`, 그리고 `cli.py:891`. 감사할 표면이 이만큼 좁다는 것이 SSOT 의
실질적 이득이다.
...
같은 `root.iterdir()` 결과다(`library.py:64-65`, `inventory.py:181`). macOS 가 이름을
...
| 손상된 기존 파일 삭제 | 비가역 | 하지 않는다 — 사람에게 알리기만 한다(`cli.py:930-932`) |
...
정직하게 적어 둘 것이 있다. `dest.exists()` 검사는 `plan_tidy`(`library.py:82`)에서
일어나고, 실제 `rename` 은 `apply_tidy`(`library.py:102`)에서 일어난다. 그 사이에
시간이 있다.
...
![바깥에서 온 문자열이 관문 하나를 지나 경로가 된다](images/32-name-boundary.svg)
...
- **치환이 만드는 이름 충돌은 `place()` 경로에서 받아 주는 곳이 없다.** `plan_tidy` 는
  ...
  시리즈 모드뿐이다(`cli.py:894-895`). 단일 실행(`_run_single`)에는 그 확인이 없다.
...
- **선행 점을 걸러내지 않는다.** `.hack//SIGN` 같은 제목은 `.hack__SIGN` 이 되어
  POSIX 에서 **숨김 파일**이 된다. 그리고 `plan_tidy`(`library.py:64`)와
  `inventory.scan`(`inventory.py:181`)이 둘 다 `not p.name.startswith(".")` 로
...
1. **파일 이름의 대부분을 원격 서버가 정한다**(`series.py:194,310-316`). 그 문자열이
   `place()` 한 줄에서 경로 성분 둘이 된다(`library.py:33`). 여기가 신뢰 경계다.

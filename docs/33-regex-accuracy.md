# 제33장 — 정규식의 정확도가 곧 오분류율

**`(?<!\d)` 하나가 막는 것**

> 제7부 표현과 이식성 · 33/33
> 선행: 제32장(파일명이라는 인터페이스) · 후행: 제34장(테스트 오라클 문제)
...
| 규칙 | 앵커 | 무엇을 포기하는가 |
|---|---|---|
| 화수 앞이 숫자면 화수가 아니다 | `naming.py:20` `(?<!\d)` | 네 자리 이상으로 표기된 화수 |
| 화수는 세 자리까지 | `naming.py:20` `\d{1,3}` | 1000화 이상 |
| 같은 시리즈가 둘 이상일 때만 폴더로 묶는다 | `library.py:75-76` | 혼자뿐인 회차 |
...
![부정 후방탐색이 화수 후보 앞 글자를 검사해 통과와 차단을 가른다](images/33-lookbehind-gate.svg)
...
![한 번의 오분류가 세 소비자로 번진다](images/33-misclassification-spread.svg)
...
|---|---|---|---|
| 시리즈 폴더 배치(신규) | `library.py:33` | `series_of` | 새로 받은 파일이 엉뚱한 폴더에 놓인다 |
| 시리즈 폴더 되모으기 | `library.py:69-71` | `split_episode` `series_of` | 무관한 파일들이 한 폴더로 묶인다 |
| 재고 조사 | `inventory.py:187,196` | `episode_of` `series_of` | 영화가 "n화 있음"으로 세어져 진짜 n화가 영영 빠진다 |
| 자막 URL 후보 | `subtitles.py:405` | `name_variants` | 없는 이름으로 주소를 조립해 후보 전부가 헛요청 |
| 이웃 화수 수집 | `cli.py:346` | `episode_names` | 범위 전체가 엉뚱한 이름으로 조립된다 |
| 출력 파일 배치 | `cli.py:663-665` | `split_episode` `series_of` | 결과물이 엉뚱한 폴더에 놓인다 |
...
성공에서 멈춘다(`subtitles.py:392-393`). 원본을 먼저 두지 않으면, 변형 후보가 우연히
...
것이고(`subtitles.py:392-393` 이 "짐작으로 넣은 경로는 실패할 때마다 헛요청이 된다"고
...
적었는지 알 길이 없다. `cli.py:347-348` 이 이 예외를 그대로 `SystemExit` 로 올린다.
...
생긴다는 것이고, 재고 조사가 그중 하나만 남긴다(`inventory.py:197-201`).

...
> 제8부 검증 방법론 · 34/39
> 선행: 제33장(정규식의 정확도가 곧 오분류율) · 후행: 제35장(결함 주입 설계)
...
![게이트 하나만으로는 가짜 구현이 통과한다](images/34-two-gates.svg)
...
|---|---|---|
| `report.py:71` — `mark = {PASS: "✓", WARN: "!", FAIL: "✗"}[self.verdict]` | 리포트 렌더링 시 FAIL 검사줄마다 | 반영된다(종료 코드 2) |
...
세 개를 시도해 하나만 받아져도 판정은 **PASS** 다. 그동안 `cli.py:358-360` 은 실패한 둘
각각에 `✗` 를 찍는다. 종료 코드 0, 판정 PASS, 그런데 로그에는 `✗` 가 둘 — `grep -q '✗'`
가 없으면 정상 케이스 시험이 이 상태를 통과시킨다.

![한 판정, 두 관측 채널 — 사각지대가 겹치지 않는다](images/34-two-channels.svg)
...
`!` 는 WARN 이다. 그리고 WARN 은 종료 코드 0 이다(`cli.py:652`).

| 주입 결함 | 지목하는 검사 | 그 검사의 판정 | 근거 | 종료 코드에 기여 |
|---|---|---|---|---|
| 세그먼트 404 | 세그먼트 수신 | FAIL | `report.py:167-173` | 있음 |
| 세그먼트 404(2차 효과) | 타임라인 연속성 | FAIL | `report.py:311-317` | 있음 |
| 200 + HTML 오류 페이지 | 페이로드 유효성 | FAIL | `report.py:199-206` | 있음 |
| TS 패킷 12개 제거 | TS 무결성 | **WARN** | `report.py:243-249` | **없음** |
| 세그먼트 중복 송출 | 세그먼트 고유성 | **WARN** | `report.py:214-218` | **없음** |
...
- **`grep -q '✗'` 는 판정의 문자열 대리(proxy)다.** `cli.py:964` 의 `--tidy` 미리보기는
...
6. `expect_pass` 가 종료 코드와 로그 `✗` 를 **함께** 보는 이유는 두 채널의 사각지대가
   다르기 때문이다. WARN 은 종료 코드 0 으로 접히고(`cli.py:651-652`), 개별 항목 실패는
   집계 판정에 도달하지 않을 수 있다(`report.py:464-469`).

...
> 제8부 검증 방법론 · 35/39
> 선행: 제34장(테스트 오라클 문제) · 후행: 제36장(대조군)
...
|---|---|
| 검사가 **호출되기는 하는가** | `cli.py:459-464` 에서 `sniff()` 결과가 `bogus` 로 쌓이지 않으면 검사 자체가 실행되지 않는다 |
| 검사 결과가 **판정으로 이어지는가** | `report.py:199-211` 에서 `bogus` 가 FAIL 로 승격되지 않으면 리포트에 나타나지 않는다 |
| 판정이 **종료 코드로 이어지는가** | FAIL 이 있어도 exit 0 이면 CI 는 아무것도 알지 못한다 |
| 검사 사이의 **상태 전달**이 맞는가 | CC 검사는 세그먼트 경계를 넘어 `state` dict 를 이어 넘긴다(`tsanalyze.py:71-83`) |
...
![8종의 결함 주입이 파이프라인의 서로 다른 여섯 층에 배치된 지도](images/35-injection-map.svg)
...
![제거 개수에 따른 CC 검출 여부와 두 사각지대](images/35-cc-blindspot.svg)
...
`None` 이 돌아오면 호출부는 보정을 **건너뛴다**(`cli.py:283-286`). 자막은 원래
...
적용했더라도 `shift()` 는 음수 시각을 0 으로 클램프하므로(`subtitles.py:227`)
...
FAIL 을 낸다(`report.py:432`). 마지막 큐가 29초, 영상이 30초이므로 오프셋이 6초를
...
기준을 끌고 간다.** 그래서 `report.py:342` 는 기준선을 플레이리스트 **선언 길이**로
...
| 경로 | 리포트 문장 | 단언 |
|---|---|---|
| 별도 파일 | `영상 범위를 벗어난 트랙 …` (`report.py:444`) | `자막 타임라인.*영상 범위를 벗어난` |
| 컨테이너 내장 | `… 영상 범위를 벗어남, X-TIMESTAMP-MAP …` (`report.py:365`) | `자막 타임라인.*영상 범위를 벗어남` |
...
(`subtitles.py:398-414`), 서버는 존재하는 `.srt` 를 정직하게 200 으로 내려준다.
...
| 파일 | 크기 | 걸리는 분기 | 사유 문자열 |
|---|---|---|---|
| 0바이트 | 0 B | `inventory.py:135-136` 크기 하한 | `0B 뿐이다 — 영상이라고 보기 어렵다` |
| 60% 잘림 | 790,939 B | `inventory.py:94-95` 상자 경계 | `mdat 상자가 파일 끝을 넘어간다 — 잘린 파일` |

`MIN_BYTES` 는 64 KB 다(`inventory.py:35`). 0바이트는 그 하한에서 즉시 걸리므로
...
![결함 넷을 함께 주입했을 때 CC 관측치가 섞이는 모습](images/35-crosstalk.svg)
...
판정 규칙은 "드리프트의 절댓값이 TARGETDURATION 이상이면 FAIL"이다(`report.py:267`).
...
- **음수 오프셋의 원인 귀속은 확인하지 못했다.** `tests/run.sh:90-91` 주석은 원인을
  ...
  파이프라인에서 매핑을 버리는 주체는 `subtitles.py:209-210` — **저장소 자신의 범위
  검사**다. 오프셋은 전 경로에서 이 함수가 계산하고 ffmpeg 에는 `-itsoffset` 으로
  넘어가므로(`subtitles.py:361-366`), ffmpeg 가 같은 값을 어떻게 다루는지는 이
  경로에서 관측되지 않는다. 주석의 규격 근거(33비트 부호 없음)는 옳지만 **주체는
  ffmpeg 이 아닐 수 있다.**
- **`n = 15` 사각지대는 이 저장소의 구현 특성이다.** `cc == prev` 를 규격 허용
  중복으로 보아 넘기는 판단(`tsanalyze.py:115`)에서 나온다. 다른 구현은 다르게

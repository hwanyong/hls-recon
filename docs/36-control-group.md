...
**"ffmpeg 은 놓친다"를 테스트로 고정하기**

> 제8부 검증 방법론 · 36/39
> 선행: 제35장(결함 주입 설계) · 후행: 제37장(양방향 고정)
...
| 처치군 | `[3/4]` — `hls-recon` 을 돌린 결과 (`run.sh:474-481`) |
| 대조군 | `[4/4]` — `ffmpeg` 단독을 돌린 결과 (`run.sh:512-522`) |
| 관측 지표 | **프로세스 종료 코드** 하나 |
| 통제한 교란 변수 | 같은 URL · 같은 로컬 HTTP 서버 · 같은 실행 · 같은 ffmpeg 바이너리 |

![통제된 비교 — 달라진 것은 계측 계층 하나뿐이다](images/36-control-vs-treatment.svg)
...
(`run.sh:477`)과 **문자 그대로 같다.** 대조군용 픽스처를 따로 만들지 않았다.

따로 만들었다면 무엇이 깨지는가.

- **결함이 같다는 보장이 사라진다.** 픽스처 생성은 `ffmpeg` 인코딩(`run.sh:37-44`)과
  파이썬 바이트 조작(`run.sh:131-146`)을 거치는데, 두 번 만들면 GOP 경계·패킷 수가
  달라질 수 있다. 그러면 "ffmpeg 이 놓쳤다"가 **결함이 더 작았기 때문**일 가능성이
  열린다 — 전형적인 교란 변수다.
- **같은 서버·같은 시각이라는 조건도 사라진다.** 404 는 서버가 파일을 못 찾아서 나는
  것이므로, 서버 상태가 다르면 애초에 다른 실험이다.

두 실행이 같은 `$WORK` 디렉터리, 같은 `python3 -m http.server`(`run.sh:149`),
...
| `set +e` … `set -e` | 실패 허용 구간 | 스크립트 첫 줄이 `set -euo pipefail`(`run.sh:8`)이므로, 감싸지 않으면 **대조군이 실패한 순간 스크립트가 즉사한다** |
...
![두 관측이 합쳐져야 한 문장이 성립한다](images/36-two-observations.svg)
...
(`run.sh:131-147`) — TS 패킷 12개 제거·세그먼트 중복·404·200 오류 페이지.
...
`127.0.0.1` 이다. 픽스처는 저장소와 같은 절차(`run.sh:37-44`)로 만들되 인코딩을
...
`probe.py:191-198` 이 서술한 성질이 그대로 재현된다.
...
3. 처치는 "ffmpeg 대신 우리"가 아니라 **"같은 ffmpeg 재조립 위에 계측 계층 하나"**
   다(`assemble.py:1-6`). 대조군과 처치군이 같은 URL·같은 서버·같은 실행을 공유하는
   것이 이 비교의 내부 타당도 전부다.

...
> 제8부 검증 방법론 · 37/39
> 선행: 제36장(대조군) · 후행: 제38장(기준선 오염과 판정 보류)
...
(`inventory.py:13-15`)이 반대 방향을 말하고, 그 방향은 제20장에서 이미 해부했다 —
...
|---|---|---|
| 0바이트 파일 | `MIN_BYTES`(`inventory.py:35`) | 아니오 — 정상 영상은 64 KB 를 넘는다 |
| 60% 에서 잘린 MP4 | 박스 경계 검사(`inventory.py:94-95`) | 아니오 — 온전한 파일은 경계가 정확히 맞는다 |
| `moov` 가 뒤에 있는 정상 파일 | 순서를 보지 않는 집합 판정(`inventory.py:96`) | 아니오 |
...
건은 이미 나간 뒤다(`inventory.py:6-10`). **손해가 작다는 것이지 없다는 것이 아니다.**
...
![안전한 실패가 퇴화하는 경로와 그것을 갈라내는 단정](images/37-degenerate-rule.svg)
...
![오탐이 실질 재현율을 0 으로 무너뜨리는 되먹임 고리](images/37-fpr-collapse.svg)
...
- **`--verify-existing`(deep) 경로는 고정되지 않는다.** `flaw(path, deep=True)` 가
  `ffprobe` 를 띄우는 분기(`inventory.py:153-159`)는 이 블록의 어느 단정도 지나가지
...
7. **"모호하면 포기한다"는 안전한 규칙은 "항상 포기한다"로 굳어도 아무 예외를 던지지
   ...
   잡는다. 실측에서 `inventory.py:245-247` 세 줄에 매달린 단정은 A9 하나였다.

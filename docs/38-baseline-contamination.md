...
![기준선 오염 — 자막이 스스로 기준선을 끌고 간다](images/38-baseline-contamination.svg)
...
| **관측 가능성** | 판정 시점에 실제로 손에 있다 | `playlist.py:172-175` 가 계산해 둔다 |
...
규칙은 그 뒤 `elif` 에 있다(`report.py:431`).
...
![판정 규칙보다 먼저 오는 전제 게이트](images/38-unknown-vs-pass.svg)
...
사실을 알린다(`cli.py:589-590`). **옵션이 지정됐다는 것과 옵션이 효과를 냈다는 것은
...
|---|---|
| 리포트 | 정상 스트림에 `✗ 자막 타임라인` |
| 종료 코드 | `FAIL` → `exit 2`(`cli.py:651-652`) |
...
경로(`report.py:358-366`)에는 `sampled` 조건이 없다.
...
5. **판정 보류** — `--limit` 표본 실행에서는 영상만 잘리고 자막은 온전히 오므로
   ...
   하지 않고 그 사실을 출력한다(`report.py:421-430`). 전제 게이트가 판정 규칙보다
   **앞에** 온다.

...
> 제8부 검증 방법론 · 39/39
> 선행: 제38장(기준선 오염과 판정 보류) · 후행: 없음 — 교재의 끝
...
![계측치가 하나의 종료 코드로 수렴한다](images/39-convergence.svg)
...
> `PASS`·`WARN`·`FAIL` 세 값의 문자열 상수다(`report.py:15`).
...
문자 수로 채우면 항목 이름이 어긋난다(`report.py:27-30`).
...
`build()` 안의 모든 `rep.add` 호출을 그대로 옮긴 것이다(`report.py:123-511`).
...
네 값이 무엇을 관측한 것인지부터 구분해야 한다(`tsanalyze.py:44-47`).
...
경계를 넘어 이어진다(`tsanalyze.py:104-119`). 여기에 `EXT-X-DISCONTINUITY` 로 예고된
...
`FAIL` 을 낸다(`report.py:441-451`). 같은 상수 쌍이 내장 경로에도 있다
(`report.py:358-366`). 제22장의 규칙 — 근거 없는 상수에는 `FAIL` 을 물리지 않는다 —
과 어긋나는 자리다.
...
(`report.py:358-366`)에는 §39.5.3 의 판정 보류에 해당하는 전제 조건이 없기 때문이다.
...
|---|---|---|
| `if fetches:` | `report.py:160` | 세그먼트 수신 · 응답 지연 · 페이로드 유효성 · 세그먼트 고유성 (**4개**) |
| `if ts and ts.parsed:` | `report.py:233` | TS 무결성 |
| `if media and media.ok:` | `report.py:263` | 길이 정합 · 스트림 구성 (**2개**) |
| `if gaps is not None and gaps.ok:` | `report.py:304` | 타임라인 연속성 |
| `if subs and (...)` | `report.py:343` | 자막 관련 전부 (**최대 6개**) |
| `if decode_errors is not None:` | `report.py:502` | 전체 디코드 |
...
`TSReport.parsed` 가 `False` 이므로(`tsanalyze.py:78-82`) 두 번째 조건절에서 다시 걸린다.
...
(`probe.py:131-137`, `probe.py:210-220`) `report.py` 어디에서도 읽히지 않는다 — 정상
실행 경로에서는 콘솔에도 찍히지 않는다(`cli.py:545` 의 출력은 `--probe-only` 전용이다).
...
![없는 항목은 통과가 아니다](images/39-absent-is-not-pass.svg)
...
`cli.py` 의 실패 경로는 전부 `raise SystemExit(문자열)` 이다(`cli.py:135` `:140`
...
`WARN` 인 회차는 요약 목록에 `WARN` 으로 인쇄되지만(`cli.py:938-939`) `failed` 를
올리지 않는다. 두 층의 정책이 일관된다.

한편 회차 층은 `FAIL` 말고도 두 가지를 실패로 센다 — 재생 소스 해석 실패
(`cli.py:888`)와 회차 처리 중의 `SystemExit`(`cli.py:921`)다. 즉 **단일 실행에서는
...
(`report.py:95-108`, `cli.py:643-646`). 여기에 제12장의 주제가 다시 나타난다.
...
편집 대상은 네 개의 헤더 이름이다(`report.py:33`). 제12장 §12.6 이 이 목록의 사정거리를
다뤘으므로 되풀이하지 않고, **판정 층에서 새로 보이는 것**만 적는다.

`_redact_headers` 는 `-headers` 다음 토큰만 손댄다(`report.py:42-46`). 그런데
...
|---|---|---|---|
| `stats.mux_command` 의 `-headers` | `report.py:45` | 요청 헤더 | **가려짐** (4종) |
| `stats.mux_command` 의 입력 URL | `report.py:149` | 서명·토큰이 붙은 미디어 URL | 그대로 |
| `stats.subtitles.tracks[].url` | `report.py:486` | 자막 트랙 URL | 그대로 |
| `stats.subtitles.extra[].url` | `report.py:475` | 이웃 화수 자막 URL | 그대로 |
| `stats.bogus_payloads[].head_hex` | `report.py:207-209` | 오류 응답의 선두 16바이트 | 그대로 |
...
(`cli.py:635`). 즉 **플레이리스트에 그 태그를 갭 수 이상 적어 넣으면 타임라인 연속성
...
(`report.py:316`). 완화 자체를 없앨 수 없다면, 최소한 **완화가 눈에 보이게** 만든다.
...
- **`target_duration` 이 0 이면 길이 정합에 `FAIL` 분기가 성립하지 않는다.**
  `if target_duration and abs(drift) >= target_duration`(`report.py:267`)의 첫 조건
...
- **같은 이름의 `Check` 가 둘 생길 수 있다.** 사이드카 자막을 내장하는 실행에서는
  `report.py:361` 과 `report.py:441`(또는 `:455`)이 모두 `자막 타임라인` 항목을
  추가한다. 집계는 이름을 보지 않으므로 판정은 옳지만, 항목 이름으로 리포트를 조회하는
  소비자는 어느 쪽을 얻을지 알 수 없다.
- **리포트 JSON 의 자막 통계 일부가 항상 덮어써진다.** `report.py:367-381` 이 내장
  전용 통계(`mode`·`streams_in_output`·`span`)를 `rep.stats["subtitles"]` 에 넣지만,
  같은 키가 `report.py:471-499` 에서 무조건 다시 대입된다. `--sub-embed` 만 쓴 실행은
  `subs.results` 가 비어 있으므로(`cli.py:610`) 최종 JSON 의 `tracks` 가 빈 배열이
